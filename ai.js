...
/* ══ Config ══ */
const AI_CONFIG = {
  API_KEY:      'AIzaSyAYqOsER-3LhPrGeT-IKGWYYBd_60mES2Q',
...
async function aiCallGemini(userPrompt) {
  ...
  // بناء المحادثة — يجب أن تبدأ بـ user وتتناوب user/model
  const rawHistory = aiState.msgs.slice(-6);
  const contents = [];
  ...
  let res;
  try {
    res = await fetch(`${AI_CONFIG.API_URL}?key=${AI_CONFIG.API_KEY}`, {
      method:  'POST',
  ...
  if (!res.ok) {
    ...
    console.error('[AI] API error:', code, errBody?.error?.message || '');
    if (code === 429) throw new Error('QUOTA_EXCEEDED');
    if (code === 400) throw new Error('BAD_REQUEST');
    if (code === 403) throw new Error('API_KEY_INVALID');
  ...
  if (!text) {
    ...
    console.error('[AI] Empty response, finishReason:', reason, data);
...
async function sendAiMsg() {
  ...
  try {
    ...
    if (err.message === 'QUOTA_EXCEEDED')  errMsg = '⛔ تجاوزت حد API اليومي';
    if (err.message === 'BAD_REQUEST')     errMsg = '⚠️ طلب غير صالح، حاول مجدداً';
    if (err.message === 'EMPTY_RESPONSE')  errMsg = '🤔 لم أتلقَّ رداً، حاول مجدداً';
    if (err.message === 'NETWORK_ERROR')   errMsg = '📡 لا يوجد اتصال بالإنترنت';
    if (err.message === 'API_KEY_INVALID') errMsg = '🔑 مفتاح API غير صالح';
    aiAddMsg('bot', errMsg);
    console.error('[AI] sendAiMsg error:', err.message, err);
  } finally {
